...
export default class QRCodeModel {
    ...
    public createMovieClip (target_mc, instance_name, depth) {
        ...
        for (var row = 0; row < this.modules.length; row++) {
             ...
             for (var col = 0; col < this.modules[row].length; col++) { 
                 var x = col * cs; var dark = this.modules[row][col]; 
    ...
    public static createData(typeNumber, errorCorrectLevel, dataList) {
        ...
        for (var i = 0; i < dataList.length; i++) { 
            ...
            QRUtil.getLengthInBits(data.mode, typeNumber)); data.write(buffer); 
        }
    ...
    public static createBytes = function (buffer, rsBlocks) {
